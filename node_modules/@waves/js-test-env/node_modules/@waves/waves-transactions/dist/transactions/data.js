...
function data(paramsOrTx, seed) {
    ...
    let computedFee;
    if (version < 2) {
        let bytes = ts_lib_crypto_1.concat(BYTE(transactions_1.TRANSACTION_TYPE.DATA), BYTE(1), BASE58_STRING(senderPublicKey), COUNT(SHORT)((x) => ts_lib_crypto_1.concat(LEN(SHORT)(STRING)(x.key), [mapType(x.value)[1]], mapType(x.value)[2](x.value)))(dataEntriesWithTypes), LONG(_timestamp));
    ...
    const bytes1 = version > 1 ? proto_serialize_1.txToProtoBytes(tx) : marshall_2.binary.serializeTx(tx);
